...
contract VAMPRebaser {
     ...
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 VAMPsSold, uint256 VAMPsFromReserves, uint256 VAMPsToReserves);
    ...
    */
    ...
    public
    {
        ...
        //TODO:
        //get volume from outside oracle 
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            VAMP.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.VAMPsToUni) {
                // if the amount from reserves > VAMPsToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                VAMP.mint(uniswap_pair, uniVars.VAMPsToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            VAMP.mint(uniswap_pair, uniVars.VAMPsToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            VAMP.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.VAMPsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.VAMPsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess VAMP in the reserve
        uint256 excess = VAMP.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
            VAMPsToUni: tokens_to_max_slippage, // how many VAMPs uniswap needs
            amountFromReserves: excess, // how much of VAMPsToUni comes from reserves
            mintToReserves: 0 // how much VAMPs protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.VAMPsToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire VAMP reserves, and minting part of total
