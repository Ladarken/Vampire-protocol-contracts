...
// Modified to work in the VAMP system

// all votes work on underlying _VAMPBalances[address], not balanceOf(address)
...
//
// Changes made by VAMP after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since VAMP supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match VAMP as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "VAMP Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(VAMP.initSupply(), 4), 100); } // 4% of VAMP

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(VAMP.initSupply(), 100); } // 1% of VAMP
    ...
    constructor(address timelock_, address VAMP_) public {
        timelock = TimelockInterface(timelock_);
        VAMP = VAMPInterface(VAMP_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(VAMP.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || VAMP.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = VAMP.getPriorVotes(voter, proposal.startBlock);
