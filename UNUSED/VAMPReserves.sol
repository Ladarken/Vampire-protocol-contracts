...
contract VAMPReserves {

    // Token that serves as a reserve for VAMP
        ...
        onlyGov
    {
        ...
        VAMPTokenInterface(VAMPAddress).decreaseAllowance(oldRebaser, uint256(-1));
        rebaser = rebaser_;
        VAMPTokenInterface(VAMPAddress).approve(rebaser_, uint256(-1));
